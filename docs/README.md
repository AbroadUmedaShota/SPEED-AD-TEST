...
- `仕様/`: サービス画面の画面仕様・機能要件一式。`premium/` 配下にプレミアム契約関連要件。管理者画面仕様の正本は `docs/リライト版仕様書/admin/`（2026-08-02 の再作成以降）。
- `画面/`: 画面単位のUI設計メモ。
...
- shared repo での新規・改訂は `docs/画面設計/仕様/`（管理者画面仕様は `docs/リライト版仕様書/admin/`）を更新します。
