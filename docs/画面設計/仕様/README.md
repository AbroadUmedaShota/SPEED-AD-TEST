...
`docs/画面設計/仕様/` は、サービス画面の要件定義・機能仕様の正本です。管理者画面の要件定義は 2026-05 に既存資料を削除して再作成する方針とし、2026-08-02 の再作成以降は `docs/リライト版仕様書/admin/` を正本とします。
...
- `admin/`: 空ディレクトリ（役目を終了）。管理者画面資料は 2026-08-02 の再作成以降 `docs/リライト版仕様書/admin/` が正本で、2026-05 時点の旧資料は `docs/アーカイブ/画面設計仕様admin_20260515/` にある。
- `premium/`: プレミアム登録・契約管理の要件群。

## 正本ルール
- 新規仕様は必ず `docs/画面設計/仕様/` に作成する。管理者画面資料のみ `docs/リライト版仕様書/admin/` に作成する（2026-08-28 に宣言を実態へ追従）。
