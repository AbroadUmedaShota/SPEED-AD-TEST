...
test('照合対象の総件数が アンケート管理の有効回答数を超えない', async ({ page }) => {
  // モックの現行規約「1回答=1名刺」を固定する検査。この規約では照合対象 ≦ 有効回答数が成り立ち、
  // SV-10233(486 > 394)・SV-10250(2,150 > 2,040) などの逆転 4 件を 2026-08-24 に修正した。
  // 件数指標の正式な定義(複数名刺・表裏の数え方)は 13号 §11 の #10 で未確定。
  // 多枚数カウントに確定した場合はこの不等号が成立しなくなるため、#10 の決着と同時に見直す
  await openScreen(page, '/03_admin/reconciliation/index.html');
